...
public class Task {
	...
	public void updateInfo(TaskWrapper wrapper){
		this.handle = wrapper.getTaskHandle();
		this.label = wrapper.getTaskLabel();
		
		Context_Structure context = contextStructure.get(wrapper.getStructureHandle());
		context.update(wrapper.getInteractionEventKind());

		//put back into the map
		contextStructure.put(wrapper.getStructureHandle(), context);
	}
